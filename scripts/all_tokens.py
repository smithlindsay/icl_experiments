...
parser.add_argument('--datadir', type=str, default='/scratch/gpfs/ls1546/icl_experiments/')
...
def checkpoint(model, epoch, optimizer, loss, run_name):
    path = f"{datadir}data/ckpt{run_name}{epoch}.pt"
    torch.save({
            'epoch': epoch,
...
torch.save(model, f"{datadir}data/all_tok_model_{run_name}.pth")

loss_history = np.array(loss_history)
np.save(f'loss_history_{run_name}', loss_history)

plt.figure()
plt.plot(loss_history)
plt.title(f'batch loss - mnist 2 classes - img size:({img_size}x{img_size}), tasks:2**{task_exp}')
plt.xlabel('batch')
plt.ylabel('loss')
plt.savefig(f'batch_loss_{run_name}.pdf')
...
def test_model(model, test_data, n_tasks, batch_size, seq_len, device='cuda', seen_tasks=True):
    correct = 0
    model.eval()
...
test_model(model, test_data, n_tasks, batch_size, seq_len, device='cuda', seen_tasks=True)
print(f'test accuracy from {run_name} model on SEEN tasks')

test_model(model, test_data, n_tasks, batch_size, seq_len, device='cuda', seen_tasks=False)
