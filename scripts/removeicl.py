...
parser.add_argument('--random_init_emb', type=str, default='', help='True = random init embeddings, else empty str')
...
if args.random_init_emb == 'True':
    random_init_emb = True
else:
    random_init_emb = False
freeze = args.freeze
...
# load a model that does exhibit ICL, load a model that does NOT exhibit ICL and put those embeddings on the model
#  that does exhibit ICL
# freeze embeddings, train and then test on unseen, the model should not exhibit ICL anymore
...
if not random_init_emb:
    # load the emb from the model that doesnt exhibit ICL
...
title_str = ''
if random_init_emb:
    title_str = ', random init emb'
else:
    title_str = ', emb tasks:2**12'
suptitle_str = ''
if freeze == 'tr':
    suptitle_str = ', froz transformer'
elif freeze == 'emb':
...
plt.title(f'loss - img size:({img_size}x{img_size}), tr tasks:2**16{title_str}')
